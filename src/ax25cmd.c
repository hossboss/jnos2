...
extern int doaxhsave (void);
extern int doaxhload (void);
#endif
...
{
	tprintf ("Usage : ax heard < show | dest > [<iface>]\n        ax heard < save | load >\n");
...
int doaxheard (int argc, char **argv, void *p)
{
	...
	{
		doaxhsave ();	/* 28Jan2020, Maiko, in axheard.c */
	}
	else if (!strcmp (argv[1], "load"))
	{
		doaxhload ();	/* 28Jan2020, Maiko, in axheard.c */
	}
#endif
...
struct iface *ifp;
{
...
#ifdef  BACKUP_AXHEARD
	/*
	 * 27Jan2020, Maiko, Now that we can save and load axheard list, we
	 * should not be surprised that negative values show up - since all
	 * of the timestamps are always compared to JNOS 'Starttime'. So to
	 * deal with this, we need to check if the time last heard is greater
	 * then the uptime of JNOS, and if so we need to add the uptime to
	 * the time last heard instead of the usual code, so that the time
	 * continues to grow each time 'ax25 heard' is called, until a new
	 * time stamp comes in of course. Confused yet ? No worried :]
	 */
	scsnap = secclock ();
	...
	if (tprintf ("%-9s   %12s    %7d", pax25 (tmp, lp->addr),
		tformat (scsnap), lp->currxcnt) == EOF)
			return EOF;
#else
        if(tprintf("%-9s   %12s    %7d",pax25(tmp,lp->addr),
